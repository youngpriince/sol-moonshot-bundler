...
class VolumeBot {
  ...
  async performSwap(solanaTracker, keypair, isBuy) {
    logger.info(
      ...
      } [${keypair.publicKey.toBase58()}] Initiating swap`
    );
  ...
  async swap(solanaTracker, keypair) {
    ...
    return buyTxid; // Only perform buy
  }

  logTransaction(txid, isBuy) {
    logger.info(
      `${isBuy ? chalk.green("[BOUGHT]") : chalk.red("[SOLD]")} [${txid}]`
    );
  }

  async sellAllTokens() {
    logger.info(
      chalk.white("[SELLING ALL] Triggered by manual file sell-trigger.txt")
    );

    const sellPromises = this.keys.map(async (key) => {
      ...
      if (sellTxid) {
        logger.info(
          `Successfully sold tokens for wallet: ${keypair.publicKey.toBase58()}`
        );
      } else {
        logger.error(
          `Failed to sell tokens for wallet: ${keypair.publicKey.toBase58()}`
        );
      }
    ...
    fs.unlinkSync("sell-trigger.txt");
    logger.info(
      chalk.white("[SELLING ALL] Completed and sell-trigger.txt removed")
    );
  }
  ...
  async run() {
    while (true) {
      ...
      if (fs.existsSync("sell-trigger.txt")) {
        logger.info("Sell trigger detected! Initiating sell for all wallets.");
        for (let wallet of this.activeWallets) {
          const keypair = Keypair.fromSecretKey(bs58.decode(wallet));
          await this.performSwap(solanaTracker, keypair, false);
        }
        fs.unlinkSync("sell-trigger.txt");
        break;
      }
